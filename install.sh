...
ensure_hypr_binds_overlay() {
  # hyprland.conf sources hypr-binds.conf; seed matrix defaults so TTY sessions
  # keep Super+Return / launcher chords before the machine UI synths overrides.
  local dest="$HOME/.config/lmdesktopplus/hypr-binds.conf"
  if [[ "$DRY_RUN" == "1" ]]; then
    log_info "[dry-run] would ensure vapor//matrix chord overlay -> $dest"
    return 0
  fi
  ensure_dir "$(dirname "$dest")"
  if [[ ! -e "$dest" ]]; then
...
bind = SUPER, A, exec, lmdesktopplus
bind = SUPER, Q, killactive,
bind = SUPER, F, fullscreen,
bind = SUPER, V, togglefloating,
bind = SUPER, ESCAPE, exec, loginctl lock-session
EOF
    log_info "Created vapor//matrix chord overlay -> $dest"
  fi
}
