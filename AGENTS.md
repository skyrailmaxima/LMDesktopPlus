...
- There is no GUI/display server on the VM, so the themed desktop itself cannot be rendered here — verification is via installer logs, unit tests, and CI artifacts (`.deb` / FreeBSD `.txz`).
