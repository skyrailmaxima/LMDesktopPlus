...
class PrintersAdapter:
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        try:
            cp = run_capture([self.lpstat, "-p", "-d"], timeout=4)
            if cp.returncode != 0:
                raise RuntimeError(cp.stderr.strip() or "lpstat failed")
            parsed = parse_lpstat(cp.stdout)
            snapshot = {
                "available": True,
                "backend": "lpstat",
    ...
    def _open_ui(self) -> dict[str, Any]:
        ...
        argv = [self.printer_ui]
        if Path(self.printer_ui).name == "xdg-open":
            argv = [self.printer_ui, "system-settings"]
        try:
            pid = spawn(argv)
